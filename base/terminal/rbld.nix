{ pkgs, config, lib, ... }:
let
  rbldFunction = pkgs.writeShellApplication # If you want syntax highlighting, use helix, it uses treesitter
  {
    name = "rbld";
    ...
    ''
      ...
      CONFIG_DIRECTORY=${config.baseVars.configDirectory}
      cd $CONFIG_DIRECTORY
...
in
{
  environment.systemPackages = lib.singleton rbldFunction;
}
